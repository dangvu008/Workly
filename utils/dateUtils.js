...
export const formatDuration = (minutes, t) => {
  ...
  if (t) {
    ...
    if (hours === 0) {
      return t("time.durationMinutesOnly", { minutes: mins });
    } else if (mins === 0) {
      return t("time.durationHoursOnly", { hours });
    } else {
      return t("time.duration", { hours, minutes: mins });
    }
  } else {
    // Fallback to hardcoded strings if no translation function
    if (hours === 0) {
      return `${mins} minutes`;
    } else if (mins === 0) {
      return `${hours} hours`;
