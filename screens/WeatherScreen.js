...
const WeatherHeader = memo(({ location, lastUpdatedTime, colors }) => (
  <View style={weatherScreenStyles.weatherHeader}>
    <Text style={[weatherScreenStyles.locationText, { color: colors.text }]}>
      {location}
    </Text>
    <Text style={[weatherScreenStyles.updateText, { color: colors.gray }]}>
      {lastUpdatedTime}
    </Text>
  </View>
));
...
const WeatherMain = memo(({ icon, temperature, description, colors }) => (
  <View style={weatherScreenStyles.weatherMain}>
    ...
    <Text style={[weatherScreenStyles.temperatureText, { color: colors.text }]}>
      {temperature}°C
    </Text>
    <Text
      style={[weatherScreenStyles.descriptionText, { color: colors.darkGray }]}
    >
      {description}
    </Text>
  </View>
...
const WeatherScreen = () => {
  const { userSettings, weatherData, updateWeatherData, updateSettings } =
    useAppContext();
  ...
  // Memoize API key
  const OPENWEATHER_API_KEY = useMemo(
    () => process.env.OPENWEATHER_API_KEY || "db077a0c565a5ff3e7a3ca8ff9623575",
    []
  );
    ...
    (dateString) => {
      ...
      try {
        ...
        return date.toLocaleTimeString(
          userSettings.language === "vi" ? "vi-VN" : "en-US",
          {
            hour: "2-digit",
            minute: "2-digit",
    ...
    async (isRefreshing = false) => {
      ...
      try {
        ...
        if (!weatherResponse.ok) {
          throw new Error(
            `${t("weather.weatherError")} (${weatherResponse.status})`
          );
        }
        ...
        if (!forecastResponse.ok) {
          throw new Error(
            `${t("weather.forecastError")} (${forecastResponse.status})`
          );
        }
        ...
        if (locationResponse.ok) {
          ...
          if (locationData.length > 0) {
            ...
            if (
              ...
              locationData[0].state !== locationData[0].name
            ) {
        ...
        if (!isRefreshing) {
          Alert.alert(t("common.success"), t("weather.updatedSuccessfully"), [
            { text: t("common.ok") },
          ]);
        }
      } catch (err) {
  ...
  const LoadingView = useMemo(
    () => (
      <View style={weatherScreenStyles.loadingContainer}>
        ...
        <Text
          style={[weatherScreenStyles.loadingText, { color: colors.darkGray }]}
        >
  ...
  const LocationDeniedView = useMemo(
    () => (
      <View style={weatherScreenStyles.errorContainer}>
        ...
        <Text
          style={[weatherScreenStyles.errorText, { color: colors.darkGray }]}
        >
          {t("weather.locationDenied")}
        </Text>
        <TouchableOpacity
          style={[
            weatherScreenStyles.retryButton,
        ...
        >
          <Text
            style={[
              weatherScreenStyles.retryButtonText,
              { color: colors.white },
            ]}
          >
  ...
  const WeatherDetails = useMemo(() => {
    ...
    return (
      <>
        ...
        <WeatherDetail
          icon="air"
          text={`${t("weather.wind")}: ${weatherData.windSpeed} m/s`}
          color={colors.primary}
        />
      </>
    );
    ...
    >
      {loading && !refreshing ? (
        ...
        <View style={weatherScreenStyles.weatherContainer}>
          ...
          <View
            style={[
              weatherScreenStyles.weatherDetails,
          ...
          {forecast.length > 0 && (
            <View style={weatherScreenStyles.forecastSection}>
              <Text
                style={[
                  weatherScreenStyles.forecastTitle,
                  { color: colors.text },
                ]}
              >
                {t("weather.forecast")}
          ...
          <TouchableOpacity
            style={[
              weatherScreenStyles.refreshButton,
              { backgroundColor: colors.primary },
            ]}
            onPress={() => fetchWeatherData()}
            disabled={loading}
          >
            ...
            <Text
              style={[
                weatherScreenStyles.refreshButtonText,
                { color: colors.white },
              ]}
            >
              {t("weather.update")}
