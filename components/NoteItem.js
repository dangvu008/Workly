...
const NoteItem = memo(({ note, onEdit, onDelete }) => {
  ...
  const renderContent = useMemo(
    () => (
      <Text
        style={themedStyles.noteContent}
        numberOfLines={3}
        ellipsizeMode="tail"
      >
        {note.content}
  ...
  const renderReminderInfo = useMemo(
    ...
    [
      ...
      t,
      themedStyles.reminderInfo,
      themedStyles.reminderText,
      colors.primary,
    ]
  );
  ...
  const renderShiftInfo = useMemo(() => {
    ...
    associatedShiftNames,
    themedStyles.shiftInfo,
    themedStyles.shiftText,
    colors.primary,
  ]);
