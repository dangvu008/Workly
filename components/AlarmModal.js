...
const AlarmModal = memo(({ visible, onDismiss, title, message, alarmSound = "alarm_1", vibrationEnabled = true }) => {
  ...
  const displayMessage = useMemo(() => message || t("alarm.timeToWork"), [message, t])
  const currentDateTime = useMemo(() => formatDate(new Date(), "full"), [])
  const displayTimeText = useMemo(() => `${t("alarm.displayTime")}: ${formatElapsedTime()}`, [t, formatElapsedTime])
  ...
  return (
    <Modal visible={visible} transparent animationType="fade" onRequestClose={handleRequestClose}>
      <View style={alarmModalStyles.modalOverlay}>
        <View style={[alarmModalStyles.modalContent, { backgroundColor: colors.card }]}>
          <View style={[alarmModalStyles.alarmIcon, { backgroundColor: colors.accent + "20" }]}>
            <MaterialIcons name="alarm-on" size={48} color={colors.primary} />
          </View>

          <Text style={[alarmModalStyles.title, { color: colors.text }]}>{displayTitle}</Text>
          <Text style={[alarmModalStyles.message, { color: colors.darkGray }]}>{displayMessage}</Text>
          <Text style={[alarmModalStyles.time, { color: colors.primary }]}>{currentDateTime}</Text>
          <Text style={[alarmModalStyles.elapsedTime, { color: colors.gray }]}>{displayTimeText}</Text>

          <TouchableOpacity
            style={[alarmModalStyles.dismissButton, { backgroundColor: colors.primary }]}
            onPress={handleDismiss}
            activeOpacity={0.7}
          >
            <Text style={[alarmModalStyles.dismissButtonText, { color: colors.white }]}>{dismissButtonText}</Text>
          </TouchableOpacity>
        </View>
      </View>
    </Modal>
